 ...
 *
 */
ArduRPCSerial::ArduRPCSerial(Stream &serial, ArduRPC &rpc)
{
  this->_serial = &serial;
  this->_rpc = &rpc;
  this->_state = 0;
 ...
 */
void ArduRPCSerial::processDataHex(uint8_t c)
{
  if(c == '\r')
    return;
 ...
 */
void ArduRPCSerial::processResultHex()
{
    uint8_t i;
    uint8_t len;
    uint8_t *data;
